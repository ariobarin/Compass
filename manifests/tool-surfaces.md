...
| Display.dev skill | Publishes HTML or Markdown to an external sharing service through HTTPS or its authenticated CLI | Partial | High | Confirm the source and audience before upload, verify the returned URL, and keep credentials, claim codes, account state, and generated artifacts outside Compass. |
