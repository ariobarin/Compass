...
Understanding what the code does and why it exists is input. Blast radius asks what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the relevant PR and commits when history is part of the change.
...
6. For a big or wide change, ask several independent subagents the same question and merge the answers. Different passes catch different real bugs.
